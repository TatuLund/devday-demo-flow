...
public class GridView extends SplitLayout {
    ...
    private FormLayout createForm() {
        ...
        form.setSizeFull();
        form.addFormItem(nameField, "Name: ").getElement()
    ...
    private TextField createLimitTextField() {
        ...
        limit.addValueChangeListener(
                event -> expensesGrid.getDataProvider().refreshAll());
    ...
    public static void scrollTo(Grid<?> grid, int index) {
        ListDataProvider<MonthlyExpense> ldp = (ListDataProvider) grid
                .getDataProvider();
        if (ldp.getFilter() == null)
    ...
    private void initalizeAndPopulateGrid(Grid<MonthlyExpense> grid) {
        ...
        grid.addColumn(TemplateRenderer.<MonthlyExpense> of(
                ...
                .setSortable(true).setAutoWidth(true).setHeader("Year")
        ...
        grid.addColumn(MonthlyExpense::getMonth).setSortable(true)
                .setHeader("Month").setKey("month").setId("month-column");
        ...
        grid.addColumn(MonthlyExpense::getExpenses).setResizable(true)
                .setKey("expenses").setHeader("Expenses")
                .setClassNameGenerator(monthlyExpense -> monthlyExpense
                        .getExpenses() >= getMonthlyExpenseLimit()
                                ? "warning-grid-cell"
                                : "green-grid-cell")
        ...
        grid.addItemClickListener(event -> {
            MonthlyExpense item = event.getItem();
        ...
        binder.forField(numberField).bind(MonthlyExpense::getExpenses,
                MonthlyExpense::setExpenses);
        ...
        // TemplateRenderer.<MonthlyExpense>of("<div style=\"border: 1px solid
        // gray; padding: 10px; width: 100%; box-sizing: border-box;\"
        // inner-h-t-m-l=\"[[item.html]]\"></div>")
        // .withProperty("html", o ->
        // Stream.of(2, 3, 4, 5, 7, 8, 9).map(i -> "Value " + i + ": " +
        // o.getYear()).collect(Collectors.joining("<br>"))
        // )
        // );
        grid.getElement()
                .executeJs("return this.getBoundingClientRect().width;")
                .then(String.class, value -> System.out.println(value));
        grid.getElement()
                .executeJs("return this.getBoundingClientRect().height;")
                .then(String.class, value -> System.out.println(value));
    }
    ...
    private void populateGridContextMenu(Grid<MonthlyExpense> grid,
        ...
        GridMenuItem<MonthlyExpense> menuItem = menu.addItem("Item", event -> {
            event.getItem()
                    .ifPresent(item -> Notification
                            .show("This is " + item.getYear() + "/"
                                    + item.getMonth(), 3000, Position.TOP_START)
                            .addThemeVariants(
        ...
        menu.addGridContextMenuOpenedListener(event -> {
            ...
            // menu.close();
            // }
            event.getColumnId().ifPresent(c -> {
                if (!c.equals("year-column"))
                    menu.close();
            });
        });
    ...
    private void addYearSelectorMenuToColumnHeader(Grid<MonthlyExpense> grid) {
        ...
        grid.getHeaderRows().get(0).getCell(grid.getColumnByKey("year"))
                .setComponent(div);
    ...
    private void addMonthFilterMenuToColumnHeader(Grid<MonthlyExpense> grid) {
        ...
        grid.getHeaderRows().get(0).getCell(grid.getColumnByKey("month"))
                .setComponent(div);
        grid.addSelectionListener(event -> {
    ...
    private void populateFilterMenu(Grid<MonthlyExpense> grid,
            ContextMenu menu) {
    ...
    private void populateContextMenu(Grid<MonthlyExpense> grid,
            ContextMenu menu) {
        for (int i = 0; i < 10; i++) {
            final int index = i;
