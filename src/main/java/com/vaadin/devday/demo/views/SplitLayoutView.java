...
public class SplitLayoutView extends SplitLayout {
    ...
    public SplitLayoutView() {
        ...
        innerLayout.addSplitterDragendListener(event -> {
            // Charts do not reflow automatically in all cases even when wrapped
            // in Div
        ...
        addSplitterDragendListener(event -> {
            // Waiting
            // https://github.com/vaadin/vaadin-split-layout-flow/issues/47 to
            // be fixed
            // if (isNarrow) {
    ...
    protected void onAttach(AttachEvent attachEvent) {
        ...
        getUI().ifPresent(ui -> listener = ui.getPage()
                .addBrowserWindowResizeListener(event -> {
                    System.out.println("Browser resize: " + this.toString());
                    if (event.getWidth() < 800) {
                        // Getting restore position would be nice
                        // https://github.com/vaadin/vaadin-split-layout-flow/issues/50
