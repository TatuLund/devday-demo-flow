...
public class MainView extends VerticalLayout implements HasUrlParameter<String>,
        HasDynamicTitle, BeforeEnterObserver {
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        Class<? extends Component> target = (Class<? extends Component>) event
                .getNavigationTarget();
        String path = null;
        for (RouteData route : registry.getRegisteredRoutes()) {
            if (route.getNavigationTarget() == target)
                path = route.getUrl();
        }
        System.out.println("Path: " + path);
        List<Class<? extends RouterLayout>> layouts = registry
                .getRouteLayouts(path, target);
